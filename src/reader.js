import {meaningHTML} from './meanings.js';
...
function askDelete(id){deleting=id;const h=state.highlights.find(h=>h.id===id);if(!h)return;$('#deleteText').textContent=h.text;let box=$('#deleteMeaning');if(!box){box=document.createElement('div');box.id='deleteMeaning';$('#deleteText').after(box);}box.innerHTML=meaningHTML(WORKS.find(w=>w.id===h.work),h,esc);$('#deleteDialog').showModal();}
...
function renderSaved(){const list=$('#savedItems');list.innerHTML=state.highlights.length?state.highlights.map(h=>{const w=WORKS.find(w=>w.id===h.work);return `<div class="saved-item"><small><span class="saved-swatch ${h.color}"></span>${esc(w?collectionOf(w)+' · '+w.title:'작품')}</small><p>${esc(excerptLabel(h))}</p>${meaningHTML(w,h,esc)}<button data-jump="${h.id}">본문에서 보기</button><button data-delete="${h.id}">삭제</button></div>`;}).join(''):'<p style="margin-top:25px">아직 표시한 문장이 없어요. 본문에서 단어나 문장을 선택하고 색을 골라 보세요.</p>';
