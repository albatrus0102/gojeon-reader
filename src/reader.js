...
function render(restore=true){
 ...
 const w=current;
 $('#main').innerHTML=`<div class="eyebrow">수능특강 문학 · ${w.author||'작자 미상'} · ${w.genre}</div><h1>${w.title}</h1><p class="meta">${w.source}</p><div class="actions"><button class="primary" id="toBody">본문 바로 읽기</button></div>
 ...
 <section class="position"><h2><span class="num">03</span>수특은 이 부분</h2><p>${w.location}</p>${w.variantNote?`<p class="source">${esc(w.variantNote)}</p>`:""}<ol class="timeline">${w.timeline.map(t=>`<li class="${t.startsWith('수특')?'current':''}">${t}</li>`).join('')}</ol></section>
 <section class="excerpt-section" id="excerpt"><h2 class="section-title"><span class="num">04</span>수능특강 본문</h2><p class="source">${w.source}<br>단어나 문장을 길게 눌러 표시할 수 있어요.</p><div class="reading" id="reading">${w.body.map((b,i)=>`<${b.type==='heading'?'h3':'p'} id="b${i}" data-block="${i}" class="${b.type}">${b.type==='dialogue'?'<span class="speaker">'+esc(b.text.slice(0,b.text.indexOf(' : ')))+'</span>'+esc(b.text.slice(b.text.indexOf(' : '))):esc(b.text)}</${b.type==='heading'?'h3':'p'}>`).join('')}</div><p class="source">— ${w.author||'작자 미상'}, 「${w.title}」</p></section>
 <section class="bottom-section" id="notes"><h2 class="section-title"><span class="num">05</span>각주·낱말</h2><dl class="notes">${w.notes.length?w.notes.map(n=>`<div><dt>${esc(n[0])}</dt><dd>${esc(n[1])}</dd></div>`).join(''):'<div class=source>이 수록 대목에는 별도로 실린 각주가 없습니다.</div>'}</dl><p class="source">수능특강 수록 각주</p></section>
 <section class="bottom-section"><h2 class="section-title"><span class="num">06</span>핵심 포인트</h2>${w.points.map(p=>`<div class="point"><h3>${esc(p[0])}</h3><p>${esc(p[1])}</p></div>`).join('')}<p class="source">${w.pointSource}</p></section><footer>본문은 업로드된 수능특강 PDF 기준입니다. 문항·선지와 문항용 기호·밑줄은 제외하고, 한자 병기와 지문·중략·각주는 유지했습니다. 줄거리·도표·보충 설명은 KBS 기준입니다.<br><br>고전 읽기 · 수능특강 독서판</footer>`;
...
function renderSaved(){const list=$('#savedItems');list.innerHTML=state.highlights.length?state.highlights.map(h=>{const w=WORKS.find(w=>w.id===h.work);return `<div class="saved-item"><small><span class="saved-swatch ${h.color}"></span>${esc(w?w.title:'작품')}</small><p>${esc(excerptLabel(h))}</p><button data-jump="${h.id}">본문에서 보기</button><button data-delete="${h.id}">삭제</button></div>`;}).join(''):'<p style="margin-top:25px">아직 표시한 문장이 없어요. 본문에서 단어나 문장을 선택하고 색을 골라 보세요.</p>';
...
$('#workSelect').innerHTML=WORKS.map(w=>`<option value="${w.id}">${w.title}</option>`).join('');$('#workSelect').onchange=e=>{savePosition();current=WORKS.find(w=>w.id===e.target.value);render();};
